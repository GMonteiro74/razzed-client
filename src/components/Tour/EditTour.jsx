...
export default function EditTour({ match }) {
    ...
    useEffect(() => {
        async function getTour() {
            const tour = await axios.get(`${process.env.REACT_APP_SERVER_HOSTNAME}/my-tours/${match.params.id}`, { withCredentials: true });
            console.log(tour);
            setType(tour.data.type);
            setLanguage(tour.data.language);
            setRate(tour.data.rate);
            setPax(tour.data.pax);
            setDescription(tour.data.description);
            setStartDate(tour.data.startDate);
            setFinalDate(tour.data.finalDate);
        }
        getTour();
    }, [])
        ...
        <div className="md:grid md:grid-cols-3 md:gap-6">
          <div className="md:col-span-1">
            <div className="px-4 sm:px-0">
              <h3 className="text-lg font-medium leading-6 text-gray-900">Update Tour</h3>
            </div>
          </div>
              ...
              <div className="shadow overflow-hidden sm:rounded-md">
                <div className="px-4 py-5 bg-white sm:p-6">
                  <div className="grid grid-cols-6 gap-6">
                    ...
                    <input type='textarea' className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" value={description} onChange={e => setDescription(e.target.value)} />
